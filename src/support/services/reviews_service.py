...
class ReviewsService(BaseService[AppUnitOfWork]):
    ...
    async def create_review(self, session_id: str, stars: int, text: Optional[str]) -> CreateReviewApiResponse:
        async with self.uow_factory() as uow:
            ...
            try:
                person, review = await gather(
                    dnr.get_person(parent.parent_id),
                    uow.review_repository.get_review(parent.parent_id)
                )
    ...
    async def resolve_review(self, review_id: int, publish: bool) -> bool:
        """
        Прохождение модерации для отзыва

        :param review_id: идентификатор отзыва (владельца отзыва)
        :param publish: опубликовать или уведомить о нарушении
        ...
        async with self.uow_factory() as uow:
            ...
            if publish:
                ...
                await uow.review_repository.open_review(review_id)
            else:
                title = "Отзыв удален"
                message = "Ваш отзыв не прошел модерацию и был удален"
                await uow.review_repository.delete_review(review_id)

            sessions = await uow.session_repository.get_sessions(review_id)
            firebase_tokens = {session.firebase_token for session in sessions}

            # Отправка уведомлений о результате модерации
    ...
    async def get_my_review(self, session_id: str) -> MyReviewApiResponse:
        async with self.uow_factory() as uow:
            ...
            # Свой отзыв можно посмотреть, даже если он на модерации
    ...
    async def get_top_reviews(
            uow: AppUnitOfWork,
            mode: Literal['likes', 'max_stars', 'min_stars'],
            offset: Optional[int],
            limit: int,
            session: Optional[Session] = None
    ) -> tuple[list[Review], set[int], int]:
        """
        Получения отзывов для просмотра

        :param uow: AppUnitOfWork для взаимодействия с БД
        :param mode: режим просмотра (сортировки) отзыва
        :param offset: смещения списка отзывов
        :param limit: лимит запроса отзывов
        :param session: сессия пользователя для получения реакций на отзывы
        :return: список отзывов, идентификаторы отзывов с реакцией, следующее смещение
        """
        ...
        # Если отзывы еще не закончились
        next_offset = None
        if len(reviews) == limit + 1:
            reviews.pop()
            next_offset = (offset or 0) + limit

        # Отзывы с реакцией пользователя
        liked_reviews = set()
        if session:
