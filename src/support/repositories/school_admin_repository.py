...
class SchoolAdminRepository(SqlAlchemyRepository[SchoolAdmin]):
    ...
    async def create_admin(self, user_id: int, name: str, parent_admin: Optional[int], person_id: Optional[int], school_id: Optional[int], dnevnik_token: Optional[str]):
        await self.create({
            'user_id': user_id,
            'name': name,
    ...
    async def delete_my_admin(self, user_id: int, admin_id):
        return await self.delete(SchoolAdmin.user_id == admin_id, SchoolAdmin.parent_admin == user_id)