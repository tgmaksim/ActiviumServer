...
class Settings(BaseSettings):
    ...
    URL_FOR_TG: str
    """Домен, который заменяет URL для Telegram-бота"""
    WWW_PATH: str
    """Путь к папке со статическими ресурсами, которые отправляются низкоуровневым сервером"""
    RESOURCES_PATH: str
    """путь к папке с дополнительными ресурсами"""
    ...
    ERROR_REAL_LOGS_PGADMIN_OPEN: str
    """Ссылка для открытия таблицы логов с реальными (не 404) ошибками в БД с параметрами {min_created_at} и {max_created_at}"""
...
def load_environ():
    """Загрузка переменных окружения из необходимого файла"""
