...
class DnevnikruApi(BaseDnevnikruApi):
    ...
    def get_school(self) -> dict:
        return self.get("schools/person-schools")

    def get_person_groups(self, person: int) -> list[dict]:
        return self.get(f"persons/{person}/edu-groups")

    def get_person_schedule(self, person: int, group: int, startDate: date, endDate: date) -> dict:
        return self.get(
    ...
    def get_work_types(self, gymnasium_id: int) -> list[dict]:
        return self.get(f"work-types/{gymnasium_id}")
    ...
    def get_homeworks(self, homeworks_id: list[int]) -> dict:
        return self.get(f"users/me/school/homeworks", homeworkId=homeworks_id)
    ...
    def get_person_marks(self, person: int, subject: int, startDate: date, endDate: date) -> list[dict]:
